# Getting Started with Food on the Fly
...
**Using conda:**
```bash
conda create -n food_on_the_fly python=3.13
conda activate food_on_the_fly
```
...
```bash
python -m food_on_the_fly.predict_model --model-path models/model.pkl --input data/test.csv
```
...
pytest tests/ --cov=food_on_the_fly
...
├── src/
│   └── food_on_the_fly/          # Importable package (src/ layout)
...
- Contact YesChef (jose.muniz@depaul.edu)
